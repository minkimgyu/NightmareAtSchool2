...
void APatrolAIController::Tick(float DeltaTime)
{
    ...
    if (MyPawn)
    {
        FVector Center = MyPawn->GetActorLocation() + FVector(0, 0, 5.f);
        FVector Forward = MyPawn->GetActorForwardVector();
        DrawSightView(Center, Forward, FColor::Orange, SightRadius);
        DrawSightView(Center, Forward, FColor::Purple, LoseSightRadius);
        DrawHearingView(Center);
    }*/
}

void APatrolAIController::DrawSightView(FVector Center, FVector Forward, FColor Color, float Range)
{
    if (SightConfig)
    {
        // 1. 기본 정보 설정 (바닥에 딱 붙지 않게 Z축으로 5정도만 띄움)
        float Radius = Range;
        float HalfAngleDeg = SightConfig->PeripheralVisionAngleDegrees;

        //UE_LOG(LogTemp, Error, TEXT("HalfAngleDeg: %f"), HalfAngleDeg);

        // 2. 부채꼴의 양 끝 직선 그리기
        FVector LeftEdge = Forward.RotateAngleAxis(-HalfAngleDeg, FVector::UpVector);
        FVector RightEdge = Forward.RotateAngleAxis(HalfAngleDeg, FVector::UpVector);

        DrawDebugLine(GetWorld(), Center, Center + LeftEdge * Radius, Color, false, -1.f, 0, 2.f);
        DrawDebugLine(GetWorld(), Center, Center + RightEdge * Radius, Color, false, -1.f, 0, 2.f);

        // 3. 부채꼴의 둥근 부분(호) 그리기
        // 10도 단위로 끊어서 선을 연결하여 곡선을 만듭니다.
        const int32 Segments = 10;
        float AngleStep = (HalfAngleDeg * 2.0f) / Segments;
        FVector PreviousPoint = Center + LeftEdge * Radius;

        for (int32 i = 1; i <= Segments; ++i)
        {
            float CurrentAngle = -HalfAngleDeg + (AngleStep * i);
            FVector CurrentDir = Forward.RotateAngleAxis(CurrentAngle, FVector::UpVector);
...
void APatrolAIController::DrawHearingView(FVector Center)
{
    if (HearingConfig)
    {
        DrawDebugCircle(
